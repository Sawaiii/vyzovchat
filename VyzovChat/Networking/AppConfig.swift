...
enum AppConfig {
    ...
    /// Использовать вымышленные данные вместо реального сервера.
    ///
    /// Включается только запуском с аргументом `-demoData` — так снимают экраны
    /// для презентации, не показывая рабочие переписки и заказы. В приложении,
    /// которое ставят сотруднику, аргумента нет, и режим недостижим.
    static let useMockData = ProcessInfo.processInfo.arguments.contains("-demoData")
