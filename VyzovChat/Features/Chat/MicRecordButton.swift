...
struct MicRecordButton: View {
    ...
    /// Насколько вести палец вверх до фиксации. Подобрано под замок: на этом
    /// расстоянии палец как раз доходит до его нижнего края, так что «довёл до
...
private struct RecordCircle: View {
    ...
    var body: some View {
        // Вид кнопки отделён от её смещения намеренно, и это главное здесь.
        //
        // Растёт кнопка пружиной, а пружина живёт около трети секунды — ровно
        // столько, сколько длится начало свайпа. Всё это время палец шлёт
        // события, и на каждом система пересобирала тело вместе с идущей
        // анимацией: сверяла масштаб круга, оба круга ореола, их прозрачность.
        // Отсюда и «первую секунду двигается рывками, дальше нормально»:
        // дальше пружина заканчивалась, и сверять становилось нечего.
        //
        // `equatable` ставит здесь границу: вид зависит только от двух флагов,
        // и пока они не менялись, система его не трогает вообще. Смещение при
        // этом применяется как было — на каждый кадр.
        RecordCircleFace(pressing: pressing, isLocked: isLocked)
            .equatable()
...
/// см. пояснение в `RecordCircle.body`.
private struct RecordCircleFace: View, Equatable {
    let pressing: Bool
    let isLocked: Bool
    ...
    /// прозрачность. Появляйся он и исчезай отдельной веткой — система
    /// пересобирала бы этот переход, и рост шёл бы ступеньками.
