...
extension View {
    /// Плашка, лежащая прямо на обоях чата: непрозрачная тёмная подложка,
    /// поверх неё — тонировка своим цветом.
    ///
    /// Обои — цвет мероприятия, и светлым он бывает вполне законно: светлота
    /// режется только до 60%. На сиреневых и мятных обоях одна тонировка в 14%
    /// исчезала вместе с содержимым — «Важное», «Жалоба» и служебные строки
    /// написаны светлым, а под ними оказывался такой же светлый фон. Тонировка
    /// хороша как оттенок, но не как единственный слой: под ней нужен свой фон,
    /// иначе читаемость плашки зависит от того, какой цвет выбрали мероприятию.
    func chatOverlay<S: Shape>(_ shape: S, tint: Color) -> some View {
        self
            .background(shape.fill(tint.opacity(0.16)))
            .background(shape.fill(Theme.bubbleOther))
    }
}
