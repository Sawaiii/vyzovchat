...
struct AdaptiveMetrics {
    ...
    ///
    /// Нулём его делать нельзя: карточки и полосы чипов прилипают к панели
    /// вплотную и читаются как её продолжение. Но и одним числом на всех
    /// неправильно — у «островка» строка состояния на 12 pt выше «брови», и
    /// панель уже уехала вниз, воздуха там и так больше. У экрана с кнопкой
    /// (SE) строка состояния низкая, отступ нужнее всего.
    var contentTopPadding: CGFloat {
        if hasDynamicIsland { return Spacing.xs }
        return hasNotch ? Spacing.s : Spacing.m
    }
