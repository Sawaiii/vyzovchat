...
struct Chat: Identifiable, Codable, Equatable, Hashable {
    ...
    var crmURL: URL? = nil
    /// Пустой акт приёма, который собирает Tony по номеру сделки: с него
    /// начинают — печатают и подписывают на площадке. Открывается в браузере,
    /// у кого есть вход в CRM; у мероприятий, заведённых руками, его нет.
    var actURL: URL? = nil
